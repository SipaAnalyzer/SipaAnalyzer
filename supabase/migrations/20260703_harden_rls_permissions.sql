...
create policy "analysis_select_permitted"
  on public.analysis for select
  using (public.has_permission('can_view_properties'));
create policy "analysis_insert_permitted"
  on public.analysis for insert
  with check (created_by_id = auth.uid() and public.has_permission('can_create_analysis'));
create policy "analysis_update_permitted"
  on public.analysis for update
  using (public.has_permission('can_edit_analysis'))
  with check (public.has_permission('can_edit_analysis'));
create policy "analysis_delete_permitted"
  on public.analysis for delete
  using (public.has_permission('can_delete_analysis'));
...
create policy "comments_select_permitted"
  ...
  using (public.has_permission('can_view_properties'));
create policy "comments_insert_permitted"
  on public.comments for insert
  with check (created_by_id = auth.uid() and public.has_permission('can_comment'));
create policy "comments_update_own_or_admin"
  on public.comments for update
  using (created_by_id = auth.uid() or public.has_permission('can_delete_property'))
  with check (created_by_id = auth.uid() or public.has_permission('can_delete_property'));
create policy "comments_delete_permitted"
...
create policy "favorites_insert_own"
  ...
  with check (created_by_id = auth.uid() and public.has_permission('can_view_properties'));
